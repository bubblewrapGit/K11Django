...
from tempapps.boardWriteForm import boardWriteForm
...
def templateFilter(request):
    ...
    num2 = 10
    # 문자형 변수
    engStr = "nakja's MustHave\r\njava <b>web</b>programming" # 특수기호+HTML태그
    hanStr = "낙자쌤의 자바 웹 프로그래밍"
    # 컬렉션형 변수
    dictVar = {'a':'유비', 'b':'관우', 'c':'장비'} # 딕셔너리
    listVar = ['손오공','저팔계','사오정']         # 리스트

    context={'num1':num1, 'num2':num2, 'engStr':engStr, 'hanStr':hanStr, 'dictVar':dictVar, 'listVar':listVar}
    # 템플릿 호출 및 값 전달
...
def formCreate(request):
    '''
    Servlet의 doGet(), doPost()와 같이 하나의 함수에서
    폼 출력과 전송된 값 처리를 동시에 하도록 권장하고 있다.
    즉, POST인 경우라면 폼값을 입력한 후 전송할때의 처리를 말한다.
    '''
    if request.method == 'POST':
        # 진입 시 전송방식이 POST라면 submit된 폼값을 처리한다.
        form = QuestionForm(request.POST)
        # 폼값의 유효성 빈값 검증을 한다.
        if form.is_valid():
            # 폼 데이터가 유효하면 클린데이터로 복사한다.
            user_id = form.cleaned_data['user_id']
            # user_id외에 title, content도 동일한 방식으로 저장할 수 있다.
...
def boardWrite(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            return render(request, 'boardWriteThx.html', {'user_name':user_name})    
    else:
        form = boardWriteForm()
    return render(request, 'boardWriteFormCreate.html', {'form':form})