...
from django.urls import include # 파일 2개일때 추가
'''
해당 URLconf는 프로젝트 레벨임으로
livepolls앱의 views파일(모듈)을 임포트해야만 접급할 수 있다.
'''
...
'''
방법1의 경우 Url패턴이 변경되면 모든 항목을 수정해야 함으로 불편하다.
하지만 방법2와 같이 파일을 2개로 나누게되면 하나의 패턴을 위한 네임스페이스가
생성되는 개념임으로 한번만 수정하면 되서 편리하다.
