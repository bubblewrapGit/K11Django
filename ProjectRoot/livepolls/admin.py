...
class ChoiceInline(admin.TabularInline):  # 테이블형으로 배치
    model = Choice  #4-1. 테이블 선택
    extra = 2       #4-2. 입력상자 갯수 

# 1. 필드 순서 변경하기 : 테이블에 적용되진 않고, 관리자모드에서만 순서가 변경됨
'''class QuestionChanger(admin.ModelAdmin):
    fields = ['pub_date', 'question_text']
'''
# 2. 필드 분히래서 보여주기 + 순서변경
'''
1번과 2번은 동시에 사용하면 에러가 발생함으로 하나만 사용해야한다.
'''
class QuestionChanger(admin.ModelAdmin):
    fieldsets = [
        ('질문을 입력하세요', {'fields':['question_text']}),
        # 3. 필드접기 : 'classes':['collapse'] 항목을 추가하면 된다.
...
'''
models.py 에서 테이블을 생성하면 관리자모드에 적용하기 위해
아래와 같이 등록절차가 필요하다.
'''
