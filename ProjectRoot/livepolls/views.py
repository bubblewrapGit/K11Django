...
def detail(request, question_id):
    # URL 패턴을 통해 전달된 일련번호로 Question테이블을 검색 후 반환한다.
...
def vote(request, question_id):
    ...
    try:
        ...
        # 예외발생 되면 detail페이지를 다시 보여준다. 이때 에러메세지를 전달한다.
        return render(request, 'livepolls/detail.html', {
            'question':question,
            'error_message' : "You didn't select a choice",
        })
    else:
        # try 절의 코드가 정상실행되면 가져온 레코드를 통해 votes항목을 +1 업데이트한다.
        ...
        '''
            reverse(urls의 별칭, 인수)
                : 별칭과 인수를 통해 URL을 만들기 위한 함수이다.
                  - 보통방식 : 요청이 있을때 URL을 분석한 후 필요한 views를 호출.
                  - reverse방식 : 별칭을 통해 거꾸로 URL을 만든다.
        '''
