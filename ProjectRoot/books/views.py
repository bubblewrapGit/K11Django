...
from django.views.generic.base import TemplateView  # 테블릿뷰
...
class BooksModelView(TemplateView): # 해당클래스는 TemplateView를 상속하여 정의
    # TemplateView를 사용하는 경우 해당 클래스 변수를 오버라이딩 해야한다.
...
제네릭뷰 중 ListView를 사용하여 리스트 구현
    : ListView를 상속받는 경우 해당 코델 클래스를 템플릿으로 전달하는 것 만으로
...
제네릭뷰 중 DetailView를 사용하여 상세페이지 구현
    : DetailView를 상속하는 경우 특정 레코드 하나를 조회한 후 컨텍스트 변수에 저장한다.
      URLConf에서 지정한 파라미터를 통해 특정 레코드 하나를 조회한 후 템플릿으로 전달하게 한다.
'''
