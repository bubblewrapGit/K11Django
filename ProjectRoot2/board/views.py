...
def list(request):
    ...
    # page번로를 통해 한 페이지에 출력할 게시물을 가져온다.
...
# 글쓰기
def write(request):
    # 전송방식이 POST라면 submit임으로 폼값을 테이블에 입력한다.
    if request.method == 'POST':
        ...
        try:
            ...
            따라서 정수를 문자열로 변경하기 위해 "문자열 + str(정수)" 형태로 기술해야 한다.
            '''
            Post.objects.create(
